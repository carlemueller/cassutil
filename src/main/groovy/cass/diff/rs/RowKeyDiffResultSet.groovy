...
class RowKeyDiffResultSet implements Rs<DiffKey> {
    List<ColumnMetadata> leftPKMeta
    List<ColumnMetadata> rightPKMeta
    ResultSet leftRS
    ResultSet rightRS
    ...
    Comparator<Row> rowComparator = new Comparator<Row>() {
        int compare(Row o1, Row o2) {
            o1.partitionKeyToken <=> o2.partitionKeyToken ?:
                    RowU.compareRowsAsObjectArrays(RowU.toArrayMD(o1,leftPKMeta),RowU.toArrayMD(o2,rightPKMeta))
        }
    }
    ...
    DiffKey one() {
        while (true) {
            ...
            if (compareResult == 0) {
                ...
                currentleftRSRow = null
                return new DiffKey(token: rowToReturn.getLong(0), key: RowU.toArrayMD(rowToReturn, leftPKMeta), side: 'LEFT')
            } else {
                lastRowSource = 'new'
                Row rowToReturn = currentrightRSRow
                currentrightRSRow = null
                return new DiffKey(token: rowToReturn.getLong(0), key: RowU.toArrayMD(rowToReturn, rightPKMeta), side: 'RIGHT')
            }
        }
    }
    ...
    boolean ready() { rightRS != null && leftRS != null}
