...
/**
 * Compares / "Diffs" two different tables. The tables must have matching row key columns (thus would produce the
 * same consistent hash values). If a key exists in both tables, it is not reported.
 *
 * COLUMN KEYS ARE NOT COMPARED WITH THIS PROGRAM
 *
...
FullKeyDiff pkdiff = new FullKeyDiff(
        left: first,
