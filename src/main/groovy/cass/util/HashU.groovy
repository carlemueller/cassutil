...
class HashU {
    ...
    static long murmur3(ByteBuffer key) {
        if (key.remaining() == 0)
    ...
    private static long normalize(long v) {
        // We exclude the MINIMUM value; see murmur3()
        v == Long.MIN_VALUE ? Long.MAX_VALUE : v
    }

    // seed appears to be 0 in the Murmur3 partitioner source code in cassandra, for all databases
    // ... this is unlikely to vary across clusters with the same version, since consistent hash values
    // are a good thing for migration of data and other operations.
    private static void hash3_x64_128(ByteBuffer key, int offset, int length, long seed, long[] result) {
